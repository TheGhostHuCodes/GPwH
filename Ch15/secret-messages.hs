...
rotNDecoder n ch = toEnum rotation
 where
  halfN    = n `div` 2
  offset   = if even n then fromEnum ch + halfN else 1 + fromEnum ch + halfN
...
newtype StreamCipher = SC Int

instance Cipher StreamCipher where
    encode (SC seed) = applySC seed
    decode (SC seed) = applySC seed
...
applySC' seed plaintext = zipWith xor randPadBits plaintextBits
 where
  randPad       = randList seed (length plaintext)
